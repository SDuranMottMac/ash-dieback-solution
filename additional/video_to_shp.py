...
class CreateSHP():
    ...
    def video_list_subfolders(self,route_path): 
        ...
        for root, folders, files in os.walk(route_path, topdown=False):
            for item in files:
    ...
    def mt_data_list_subfolders(self,route_path,in_folders): 
        ...
        if len(in_folders)== 1:
            ...
            prev_fold = in_folders[f_index-1]
            next_fold = in_folders[f_index+1]
            paths = [prev_fold,route_path,next_fold]
            mt_list = self.mt_data_collection(paths)
            return(mt_list)
