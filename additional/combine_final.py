...
'''
1) make folders to merge into X
2) merge monitoring X
3) make SS\False Positive X
4) move False Positive folders to folder and output folders (merge csv?)X
5) merge detections X
6) merge no bbox X
7) merge merge shapefile X

keep folders now - do not delete
add skip unzipped'''
...
class combine():
    def __init__(self,input_path,angle,skip,client) -> None:
        self.input_path = input_path
        
        self.shapefile_Fin = os.path.join(input_path,client+'-Final-'+angle,'Shapefile')
        self.detections_Fin = os.path.join(input_path,client+'-Final-'+angle,'Detections')
        self.nbbox_Fin = os.path.join(input_path,client+'-Final-'+angle,'No_bbox')
        self.monitoring_fin = os.path.join(input_path,client+'-Final-'+angle,'Monitoring')
        self.SS_Fin = os.path.join(input_path,client+'-Final-'+angle,'SS')
        self.angle = angle
        self.skip = skip
    ...
    def FR(self):
        for root,fold,files in os.walk(self.detections_Fin):
            for item in files:
    ...
    def merge_SS(self):
        ...
        for path in tqdm(self.glob_,desc='Merging SS folders'):
            ...
            for root,fold,files in os.walk(FP_detect):
                for item in files:
                    shutil.move(os.path.join(root,item),os.path.join(FP,'Detections',item))
                    shutil.move(os.path.join(FP_nobbox,item),os.path.join(FP,'No_bbox',item))
    ...
    def change_shp_name(self,path):
        shp_name = self.client +'-'+self.angle
        for root,fold,files in os.walk(path):
            for item in files:
                exten = item[-4:]
    ...
    def merge_shp(self):
        ...
        for path in tqdm(self.glob_,desc='Merging Shapefiles'):
            ...
            #move no change
            for root,fold,files in os.walk(os.path.join(path,'SS','Shapefiles','no_change')):
                for item in files:
                    shutil.move(os.path.join(root,item),os.path.join(self.SS_Fin,'Shapefiles','no_change'))

            for root, fold, files in os.walk(shapefile_p):
                for item in files:
                    #copy edited Shapefiles
                    shutil.copy(os.path.join(root,item),os.path.join(self.SS_Fin,'Shapefiles','Seperate_Shapefiles',item))
            
            #merge og shapefiles
            p_og_shp = os.path.join(path,'SS','og_shapefile')
    ...
    def clean_up(self):
        #clean all old paths 
        for path in tqdm(self.glob_,desc='Removing old route Directories'):
            shutil.rmtree(path)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--folders', type=str, default=None, help='input path for all routes, ie glasgow downloads folder')
    parser.add_argument('--angle', type = str,default=None,help='Front or rear')
    parser.add_argument('--client',type = str,default=None,help='client name ie. glasgow, conwy')
    opt = parser.parse_args()
    
    skip = ['Unzipped','Final']
    combiner = combine(input_path=opt.folders,angle=opt.angle,skip = skip,client = opt.client)
    combiner.Run()
